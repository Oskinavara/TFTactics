  ...
  methods: {
    filteredChampions(array) {
      return this.filterDataBySearch(
        this.filterChampionsByCost(
          this.filterChampionsByOrigin(this.filterChampionsByClass(array))
        )
      )
    },
    filterDataBySearch(array) {
      if (!this.inputValue) {
        ...
        let newArray = array.filter(item =>
          item.toUpperCase().includes(this.inputValue.toUpperCase())
        )
        return newArray
      }
    },
    filterChampionsByCost(array) {
      if (!this.filters.cost.length) {
        ...
        return array.filter(
          champion => this.champions[champion].cost === this.filters.cost[0]
        )
    },
