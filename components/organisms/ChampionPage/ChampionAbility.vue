<template>
  <section class="champion-ability">
    <heading-underlined style="margin-top: 0">Abilities</heading-underlined>
    <div class="champion-ability__wrapper">
      ...
      <div class="champion-ability__data">
        <div class="champion-ability__header">
          ...
          <div class="champion-ability__mana" v-if="champion.ability.manaCost">
            <img
              ...
              alt="mana icon"
              class="champion-ability__mana-icon"
            />
            <span>
        ...
        <p class="champion-ability__description">{{champion.ability.description}}</p>
        <ul class="champion-ability__stats">
          <li
            class="champion-ability__stat"
            v-for="stat in champion.ability.stats"
            :key="stat.type"
          >
...
export default {
  ...
  computed: {
    abilityUrl() {
      let abilityName = this.champion.ability.name
        .toLowerCase()
        .replace(' ', '-')
        .replace(' ', '-')
        .replace(/([!'])+/g, '')
      let championName = this.champion.name
        .toLowerCase()
        .replace(' ', '-')
        .replace(/(['])+/g, '')
