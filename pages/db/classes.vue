<template>
  <div>
    ...
    <custom-table :columns="columns" :tableData="classes">
      ...
      <template v-slot:[columns[2].name]="{row}">
        <div class="champion-list">
          <champion-icon
            v-for="champion in classChampions(row)"
            :key="champion.name"
            :champion="champion"
          />
        </div>
...
export default {
  ...
  methods: {
    ...
    classChampions(origin) {
      let champions = { ...this.champions }
      for (let champion in champions) {
        if (!champions[champion].class.includes(origin.name)) {
